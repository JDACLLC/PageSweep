...
function update(data){const p=Math.max(3,Math.min(100,data.progressPercent||3));status.textContent=data.status||'Capturing…';bar.style.width=`${p}%`;robot.style.left=`${p}%`;robot.style.top=`${topAt(p)}px`;robot.style.transform=`translateX(-${p}%)`;if(data.state==='complete'){card.classList.add('complete');if(completionCloseTimer===null)completionCloseTimer=setTimeout(()=>window.close(),1700)}}
