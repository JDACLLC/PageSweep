...
| 14 | P0 | In Progress | Stop long-page capture when scrolling no longer advances and prevent duplicate final frames from reaching stitching. |
| 15 | P0 | In Progress | Prevent capture status from displaying a current frame number greater than its estimated total. |
...
- Items 14 and 15 were raised by a 2026-09-13 test of an approximately 20-frame page. The card advanced through `19 of 20`, `20 of 20`, and then `21 of 20`, `22 of 20`, and `23 of 20`; capture finished scrolling but never displayed the green completion state. The working diagnosis is that Chrome rounded or clamped the final scroll position slightly below PageSweep's calculated boundary, allowing repeated captures at the same effective position. Duplicate adjacent `scrollY` values would produce a zero-height unique region and cause the existing stitch validation to reject the frame. Confirm this diagnosis with frame-position diagnostics before recording the issue as fixed.
